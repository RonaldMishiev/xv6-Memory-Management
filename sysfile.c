...
// implementation of wmap system call
uint sys_wmap(void) {
  ...
  return (uint)wmap(addr, length, flags, fd);
}
...
// wremap system call
int sys_wremap(void) {
  int addr, oldLength, newLength, flags;
  if (argint(0, &addr) < 0) {
    return -1;
  }
  // Integer arguements
  if (argint(1, &oldLength) < 0 || argint(2, &newLength) < 0 || argint(3, &flags) < 0) {
    return -1;
  }
  return wremap(addr, oldLength, newLength, flags);
}
